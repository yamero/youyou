<?php
class RechargecardController extends ControllerBase{
    public function indexAction($pageNum){
        ...
        $phql="select re.* from RechargeCard as re";
        $rechargecard=$this->modelsManager->executeQuery($phql);
    ...
    public function addAction(){
        ...
        $this->view->setVar('operation',"rechargecardadd");
    }
    public function doaddAction(){
        $data=$this->request->getPost();
        $data['create_time']=time();
        $rechargecard=new RechargeCard();
        if($rechargecard->save($data)){
            $this->response->redirect('/rechargecard/index');
        }else{
            echo "添加不成功，请检查输入！";
        }
    }
    public function delAction($id){
        $rechargecard=RechargeCard::findFirst($id);
        if($rechargecard->delete()){
            $this->response->redirect('/rechargecard/index');
        }else{
            echo "抱歉，无法删除！";
        }
